...
- (BOOL)establishTunnel {
    ...
    // send to
    [self.delegate openConnectVPNAdapter:self configureTunnelWithNetworkSettings:networkSettings completionHandler:completionHandler];
    ...
    if (self.packetFlowBridge && [self.packetFlowBridge configureSocketsWithError:&socketError]) {
        ...
        if (socketError) { [self.delegate openConnectVPNAdapter:self handleError:socketError]; }
        return NO;
    }
}
